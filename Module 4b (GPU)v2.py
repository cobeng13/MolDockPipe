#!/usr/bin/env python3
# Module 4b (GPU) — Mini-batch with idempotent resume + graceful stop

from __future__ import annotations
import csv, hashlib, re, shlex, shutil, signal, subprocess, sys
from pathlib import Path
from datetime import datetime, timezone
from typing import Dict, Optional, Tuple, Iterable

# ------------ Tunables ------------
BATCH_SIZE = 64          # how many ligands per GPU batch
SAFE_RESUME = True       # skip ligands that already have *_out.pdbqt
KEEP_TMP = False         # keep per-batch temp folders (debug)
# ----------------------------------
...
def main():
    ...
    # SAFE_RESUME: skip ligands that already have *_out.pdbqt
    if SAFE_RESUME:
    ...
    if not pending:
        print("✅ Nothing to do — all ligands already have outputs. Building summaries…")
        m=load_manifest(); build_and_write_summaries(m); return
    ...
    try:
        for bi, batch in enumerate(chunked(pending, BATCH_SIZE), 1):
            if STOP_REQUESTED or HARD_STOP:
                print("🧾 Stop requested — exiting before launching next batch.")
                break

            # make a small tmp ligand folder containing only this batch
            tmp_dir = tmp_root / f"b{bi:04d}"
            if tmp_dir.exists(): shutil.rmtree(tmp_dir, ignore_errors=True)
            tmp_dir.mkdir(parents=True, exist_ok=True)

            # to avoid admin/symlink issues on Windows, copy small files
            for lig in batch:
                shutil.copy2(lig, tmp_dir / lig.name)

            rc = run_batch(vgpu, cfg_file, tmp_dir, out_dir, gcfg)
            if rc != 0:
                print(f"⚠️ Batch {bi} exited with rc={rc}. Harvesting what’s produced then stopping.")
                # fall through to harvest; then break
            # Harvest: update manifest for any *_out.pdbqt from this batch
            for lig in batch:
                lig_id = lig.stem
            ...
            if rc != 0:
                break  # stop after harvesting this failing batch
