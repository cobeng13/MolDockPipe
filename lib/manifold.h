...
#include "ssd.h"
/*结构体ssd类型：ssd_par   结构体scale类型：exclusion
uint ：num_phases、num_phases、num_phases、num_phases
double：manifold_lambda、cost_factor、max_prob、rstart_fraction、min_rmsd、relative_pair_cost
double类型长度为三的数组hunt_cap       use_ssd   bool


print函数  : 打印  
{ssd: evals=   , initial_factor=    , min_factor=     , up=     , down=   }
num_phases=    ,
num_steps=     ,
num_attempts=  ,
 num_final_tunings=    ,
...
struct manifold {
	...
	manifold() 
		: num_phases(800), num_steps(50), num_attempts(10), num_final_tunings(1),
	      ...
	      use_ssd(false) {}
/*声明运算符重载函数 重载（） 返回类型为output_type结构体
*/
